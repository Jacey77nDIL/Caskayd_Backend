...
class ChatService:
    ...
    async def send_message(current_user_email: str, current_user_role: str, 
        ...
        query = select(Conversation).where(Conversation.id == data.conversation_id)
        result = await db.execute(query)
        conversation = result.scalar()
    ...
    @staticmethod
    async def mark_messages_as_read(conversation_id: int, current_user_role: str, db: AsyncSession = Depends(get_db)):
